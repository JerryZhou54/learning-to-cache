...
def log_validation(args, model, device, vae):
...
def main(args):
    ...
    if args.accelerate_method is not None and args.accelerate_method == "dynamiclayer":
        ...
        # from models.router_models import DiT_models
    else:
    ...
    #     model.load_ranking(args.path, args.num_sampling_steps, diffusion.timestep_map, args.thres)
        # model.add_router(args.num_sampling_steps, diffusion.timestep_map, args.path)
    
    # Auto-download a pre-trained model or load a custom DiT checkpoint from train.py:
    ckpt_path = args.ckpt or f"DiT-XL-2-{args.image_size}x{args.image_size}.pt"
    state_dict = find_model(ckpt_path)
    model.load_state_dict(torch.load("/data/hyou37_data/learning-to-cache/DiT/results/001-DiT-XL-2/checkpoints/epoch2_0096000.pt")["model"], strict=False)
    # model.load_state_dict(state_dict, strict=False)
    kept_mod_ratios = []
    for i in range(28):
        kept_mod_ratios.append(model.blocks[i].diff_mod_ratio.item())
    print(kept_mod_ratios)
    print(sum(kept_mod_ratios) / 28)
    ...
    # class_labels = [207, 992, 387, 974, 142, 979, 417, 279]
    np.random.seed(0)
    class_labels = np.random.randint(0, high=1000, size=64).tolist()
    ...
    samples, _ = samples.chunk(2, dim=0)  # Remove null class samples
    samples = vae.decode(samples / 0.18215).sample
    save_image(samples, f"Sample_NFE{args.num_sampling_steps}_Method_{args.accelerate_method}(1).png", nrow=8, normalize=True, value_range=(-1, 1))
